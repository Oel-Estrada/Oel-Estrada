    ...
    referenceFiles.forEach(file => {
        ...
        LANGUAGES.forEach(lang => {
            ...
            if (!fs.existsSync(langPath)) {
                console.error(`${LOG_PREFIX} ${COLORS.red}✖ [${lang}] El archivo ${COLORS.bright}${file}${COLORS.reset}${COLORS.red} no existe.${COLORS.reset}`);
                hasMissing = true;
                return;
            }
            ...
            if (missing.length > 0) {
                console.error(`${LOG_PREFIX} ${COLORS.red}✖ [${lang}] Claves faltantes en ${COLORS.bright}${file}${COLORS.reset}${COLORS.red}:${COLORS.reset}`);
                missing.forEach(key => console.error(`  ${COLORS.red}- ${key}${COLORS.reset}`));
                hasMissing = true;
            }

            if (extra.length > 0) {
                console.warn(`${LOG_PREFIX} ${COLORS.yellow}⚠ [${lang}] Claves extra en ${COLORS.bright}${file}${COLORS.reset}${COLORS.yellow} (no están en el de referencia):${COLORS.reset}`);
                extra.forEach(key => console.warn(`  ${COLORS.yellow}+ ${key}${COLORS.reset}`));
            }
        });
    });
