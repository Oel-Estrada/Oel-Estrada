 ...
 */
async function fillMissingInteractive(ref, target, lang, file, ask, REFERENCE_LANG, prefix = '') {
    const result = {...target};

    for (const key in ref) {
        const fullKey = prefix ? `${prefix}.${key}` : key;
        if (typeof ref[key] === 'object' && ref[key] !== null && !Array.isArray(ref[key])) {
            result[key] = await fillMissingInteractive(ref[key], target[key] || {}, lang, file, ask, REFERENCE_LANG, fullKey);
            ...
            console.log(`${COLORS.dim}Clave:${COLORS.reset} ${COLORS.bright}${fullKey}${COLORS.reset}`);
            console.log(`${COLORS.dim}Texto original (${REFERENCE_LANG}):${COLORS.reset} ${COLORS.green}${ref[key]}${COLORS.reset}`);

            const answer = await ask(`${COLORS.bright}${COLORS.cyan}Introduce la traducción${COLORS.reset} (deja vacío para "[TODO] ${ref[key]}"): `);
    ...
    try {
        for (const file of referenceFiles) {
            ...
            for (const lang of LANGUAGES) {
                ...
                const syncedContent = await fillMissingInteractive(refContent, langContent, lang, file, ask, REFERENCE_LANG);

                fs.writeFileSync(langPath, JSON.stringify(syncedContent, null, 2) + '\n', 'utf8');
                console.log(`${LOG_PREFIX} ✅ Sincronizado: ${COLORS.green}${lang}/${file}${COLORS.reset}`);
            }
        }

        console.log(`\n${LOG_PREFIX} ${COLORS.bright}${COLORS.yellow}⚠ Los cambios se han aplicado temporalmente.${COLORS.reset}`);
        const confirm = await ask(`${COLORS.bright}${COLORS.cyan}¿Deseas confirmar los cambios? (s/n/c):${COLORS.reset} `);

        if (confirm.toLowerCase() === 'c' || confirm.toLowerCase() === 'cancelar') {
